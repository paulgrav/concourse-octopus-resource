...
class ResourceType:
    ...
    def _metadata_from_deployment(self, deployment):
        return [
            ...
            { "name": "Deployment", "value": f"{self.octopus_server_uri}{deployment['Links']['Web']}" },
            { "name": "Release", "value": f"{self.octopus_server_uri}{deployment['Links']['Release']}" }
          ]
    ...
    def concourse_out(self, filepath):
        ...
        f = open(os.path.join(filepath, self.params.get("path"), "deployment.json"), "r")
    ...
    def concourse_check(self):
        ...
        items = self._latest_deployments_since_deploymentid(response.json(), self.version)
    ...
    def _get_variables(self, path):
        ...
        for i in response.json()['Variables']:
            var_name = i['Name'].replace(" ", "_" ).replace("[", "." ).replace("]","")
